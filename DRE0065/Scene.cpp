...
void Scene::setupScene2()
{
	...
	TransformationComposite* loginTransform = new TransformationComposite();
    loginTransform->addComponent(new Scale(vec3(2.0f, 2.0f, 2.0f)));
	loginTransform->addComponent(new Translation(vec3(0.0f, 1.5f, -5.0f)));
	loginTransform->addComponent(new Rotation(glm::radians(180.0f), vec3(0.0f, 1.0f, 0.0f)));
    loginObject->transformationcom = *loginTransform;
    ...
    houseTexture = SOIL_load_OGL_texture("house.png", SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y);
    if(!houseTexture) cerr << "Failed to load texture: house.png" << endl;
    else cerr << "Texture house.png loaded successfully, ID: " << houseTexture << endl;
    ...
    for(int i=0; i < numBushes; i++)
    {
        ...
        DrawableObject* bushObject = new DrawableObject(bushModel, bushShader, 8730, GL_TRIANGLES, false);
...
void Scene::setupScene5()
{
    ...
    grassTextureNight = SOIL_load_OGL_texture("grass_zpg.jpg", SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y);
    if(!grassTextureNight) cerr << "Failed to load texture: grass_zpg.jpg" << endl;
    else cerr << "Texture grass_zpg.jpg loaded successfully, ID: " << grassTextureNight << endl;
    ...
    */
    ...
    for(int i=0; i < numTrees; i++)
    {
        ...
        Model* treeModel = new Model();
        treeModel->setupModel(tree, sizeof(tree), 92814, true, false);
        DrawableObject* treeObject = new DrawableObject(treeModel, treeShaderNight, 92814, GL_TRIANGLES, false);
        treeObject->material = Material(vec3(0.05f, 0.05f, 0.05f), vec3(0.1f, 0.3f, 0.1f), vec3(0.1f, 0.1f, 0.1f), 32.0f);
        float randomScale = treeScaleDist(generator);
        float randomRotation = glm::radians(rotationDist(generator));
    ...
    for(int i=0; i < numBushes; i++)
    {
        ...
        DrawableObject* bushObject = new DrawableObject(bushModel, bushShaderNight, 8730, GL_TRIANGLES, false);
        bushObject->material = Material(vec3(0.08f, 0.18f, 0.08f), vec3(0.2f, 0.5f, 0.2f), vec3(0.05f, 0.05f, 0.05f), 64.0f);
        float randomScale = bushScaleDist(generator);
        float randomRotation = glm::radians(rotationDist(generator));
